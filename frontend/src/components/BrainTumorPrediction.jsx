...
import { CheckCircle, AlertTriangle, Info, Brain, Image as ImageIcon } from "lucide-react";
...
export default function BrainTumorPrediction() {
  ...
  const riskColor = prediction ? (prediction.prediction === 'notumor' ? "green" : "red") : "slate";
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13.5, color: "#64748b" }}>Upload an MRI scan to get an AI-powered likelihood assessment.</p>
      </div>
      ...
      <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 12, padding: "12px 16px", marginBottom: 20, display: "flex", alignItems: "center", gap: 10, fontSize: 13 }}>
        ...
        <span style={{ color: "#1e40af" }}>The model analyzes MRI scans to detect and classify common types of brain tumors.</span>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 20, alignItems: "flex-start" }}>
        <SectionCard>
          <div style={{ padding: "18px 20px", borderBottom: "1px solid #f8fafc", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 500, color: "#0f172a" }}>Upload MRI Scan</div>
            </div>
            {file && (
          ...
          <form onSubmit={handleSubmit} style={{ padding: 20 }}>
            ...
            >
              ...
              <label htmlFor="image-upload" style={{ cursor: "pointer", display: "block" }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 500, color: "#475569" }}>Click or drag to upload MRI scan</div>
                    <div style={{ fontSize: 13, color: "#94a3b8", marginTop: 4 }}>JPG, PNG, BMP, or TIFF</div>
                  </div>
                </div>
              </label>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
              <button type="submit" disabled={!file || loading} style={{ flex: 1, padding: "11px", background: (!file || loading) ? "#94a3b8" : "#3b82f6", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 500, color: "#fff", cursor: (!file || loading) ? "not-allowed" : "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                {loading ? <><div style={{ width: 16, height: 16, border: "2px solid rgba(255,255,255,0.3)", borderTop: "2px solid #fff", borderRadius: "50%", animation: "spin 0.7s linear infinite" }}></div>Analyzing...</> : "Analyze Scan"}
              </button>
          ...
          {prediction ? (
              ...
              <div style={{ padding: 20 }}>
                ...
                <div style={{ background: riskBg[riskColor], border: `1.5px solid ${riskBorder[riskColor]}`, borderRadius: 14, padding: "24px 16px", textAlign: "center", marginBottom: 16 }}>
                  <div style={{ display: "flex", justifyContent: "center", marginBottom: 8 }}>
                    {riskColor === "green" ? <CheckCircle size={36} color="#166534" /> : <AlertTriangle size={36} color="#991b1b" />}
                  </div>
                  <div style={{ fontSize: 20, fontWeight: 600, color: riskText[riskColor], letterSpacing: "-0.01em" }}>{prediction.display}</div>
                  <div style={{ fontSize: 14, fontWeight: 500, color: riskText[riskColor], marginTop: 2 }}>{riskColor === 'green' ? 'No Tumor Detected' : 'Tumor Detected'}</div>
                  <div style={{ fontSize: 13, color: riskText[riskColor], opacity: 0.8, marginTop: 10, lineHeight: 1.5 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", background: "#f8fafc", borderRadius: 10, marginBottom: 8 }}>
                  ...
                  <span style={{ fontSize: 15, fontWeight: 600, color: "#0f172a" }}>{(prediction.probability).toFixed(1)}%</span>
                </div>
                
                {Object.entries(prediction.probabilities || {}).map(([label, pct]) => (
                   <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", background: "#f8fafc", borderRadius: 10, marginBottom: 8 }}>
                    <span style={{ fontSize: 13, color: "#64748b" }}>{label}</span>
                    <span style={{ fontSize: 15, fontWeight: 600, color: "#0f172a" }}>{(pct).toFixed(1)}%</span>
                  </div>
                ))}
                ...
                <div style={{ marginTop: 14, padding: "10px 12px", background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 10, fontSize: 12, color: "#92400e", lineHeight: 1.5 }}>
                  <strong>Note:</strong> Research & Demo Use Only. Not a certified medical device.
                </div>
              </div>
            </SectionCard>
          ) : (
            <SectionCard>
              <div style={{ padding: 32, textAlign: "center" }}>
                <Brain size={48} color="#cbd5e1" style={{ display: "block", margin: "0 auto 12px" }} />
                <div style={{ fontSize: 14, fontWeight: 500, color: "#475569", marginBottom: 4 }}>Ready to analyze</div>
                <div style={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.5 }}>Upload an MRI scan to see the model's prediction</div>
              </div>
            </SectionCard>
          )}
        </div>
                ...
                {history.map((r, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid #f8fafc" }}>
                    ...
                    <td style={{ padding: "11px 16px" }}>
                      <span style={{ display: "inline-flex", padding: "3px 10px", fontSize: 12, fontWeight: 500, borderRadius: 999, background: r.prediction === 'notumor' ? "#dcfce7" : "#fee2e2", color: r.prediction === 'notumor' ? "#166534" : "#991b1b" }}>
                        {r.prediction.charAt(0).toUpperCase() + r.prediction.slice(1)}
                      </span>
                    </td>
                    <td style={{ padding: "11px 16px", color: "#1e293b", fontWeight: 500 }}>{(r.probability).toFixed(1)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
