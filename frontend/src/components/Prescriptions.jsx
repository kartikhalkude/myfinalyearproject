        ...
        <div className="dm-list-surface" style={{ background: dark ? "#111827" : "#fff", border: `1px solid ${dark ? "#334155" : "#e2e8f0"}`, borderRadius: 16, overflow: "hidden", boxShadow: dark ? "0 10px 28px rgba(2,6,23,0.35)" : "0 4px 12px rgba(15,23,42,0.03)" }}>
          {filtered.map((rx, idx) => {
            ...
            return (
              ...
              >
                <div className="dm-icon-chip" style={{ width: 40, height: 40, borderRadius: 10, background: dark ? "#0f172a" : "#f8fafc", border: `1px solid ${dark ? "#334155" : "#e2e8f0"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}>
                  <Pill size={18} color="#10b981" />
                </div>
 
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <div className="dm-soft-text" style={{ fontSize: 14, fontWeight: 700, color: dark ? "#e2e8f0" : "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{rx.diagnosis || "Prescription"}</div>
                    ...
                    <span style={{ padding: "2px 8px", fontSize: 10, fontWeight: 700, borderRadius: 999, background: st.bg, color: st.color, border: `1px solid ${st.bg}`, textTransform: "capitalize" }}>{rx.status}</span>
                  </div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "6px 12px", fontSize: 12 }}>
                    <span style={{ color: dark ? "#94a3b8" : "#64748b", fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 180 }}>
                      {user?.role === "doctor" ? `Pat: ${rx.patientId?.name || "Unknown"}` : `Dr. ${rx.doctorId?.name || rx.doctorName || "Unknown"}`}
                    </span>
                    <span style={{ color: dark ? "#475569" : "#cbd5e1", fontSize: 10 }}>•</span>
                    <span style={{ color: dark ? "#64748b" : "#94a3b8", whiteSpace: "nowrap" }}>{new Date(rx.prescribedDate || rx.createdAt).toLocaleDateString()}</span>
                  </div>
                </div>
                  ...
                  <div style={{ display: "flex", gap: 4 }} onClick={e => e.stopPropagation()}>
                    {user?.role === "doctor" && (
                      <button onClick={() => { setEditing(rx); setForm({ patientId: rx.patientId?._id || rx.patientId, diagnosis: rx.diagnosis || "", medicines: rx.medicines || [{ name: "", dosage: "", frequency: "", duration: "" }], advice: rx.advice || "", validUntil: rx.validUntil ? new Date(rx.validUntil).toISOString().split("T")[0] : "" }); setShowModal(true); }}
                        style={{ width: 30, height: 30, background: dark ? "#1e293b" : "#fff", border: `1px solid ${dark ? "#334155" : "#e2e8f0"}`, borderRadius: 8, cursor: "pointer", color: "#3b82f6", display: "flex", alignItems: "center", justifyContent: "center" }}><Edit2 size={13} /></button>
                    )}
                    <button onClick={() => handleDelete(rx._id)}
                      style={{ width: 30, height: 30, background: dark ? "#1e293b" : "#fff", border: `1px solid ${dark ? "#334155" : "#e2e8f0"}`, borderRadius: 8, cursor: "pointer", color: "#ef4444", display: "flex", alignItems: "center", justifyContent: "center" }}><Trash2 size={13} /></button>
                  </div>
          ...
          <div className="dm-modal-surface" style={{ background: dark ? "#111827" : "#fff", border: dark ? "1px solid #334155" : "none", borderRadius: 24, width: "100%", maxWidth: 640, maxHeight: "90vh", overflowY: "auto", boxShadow: dark ? "0 24px 64px rgba(2,6,23,0.55)" : "0 24px 64px rgba(15,23,42,0.2)" }}>
            <div className="dm-modal-header" style={{ padding: "24px", borderBottom: `1px solid ${dark ? "#1e293b" : "#f1f5f9"}`, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: dark ? "#111827" : "#fff", zIndex: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: dark ? "rgba(16,185,129,0.15)" : "#f0fdf4", display: "flex", alignItems: "center", justifyContent: "center", color: "#10b981" }}>
                  <Pill size={20} />
                </div>
                <div>
                  <div className="dm-page-title" style={{ fontSize: 18, fontWeight: 700, color: dark ? "#f8fafc" : "#0f172a" }}>Prescription Details</div>
                  <div className="dm-soft-muted" style={{ fontSize: 13, color: "#64748b" }}>{viewing.diagnosis || "General Consultation"}</div>
                </div>
              </div>
              <button onClick={() => setViewing(null)} style={{ width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", background: dark ? "#1e293b" : "#f8fafc", border: `1px solid ${dark ? "#334155" : "#e2e8f0"}`, borderRadius: 10, cursor: "pointer", color: "#64748b" }}><X size={18} /></button>
            </div>

            <div style={{ padding: 24 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
                <div className="dm-detail-panel" style={{ padding: 12, background: dark ? "#0f172a" : "#f8fafc", borderRadius: 12, border: `1px solid ${dark ? "#1e293b" : "#f1f5f9"}` }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", marginBottom: 4 }}>Prescribed Date</div>
                  <div className="dm-soft-text" style={{ fontSize: 14, fontWeight: 500, color: dark ? "#f1f5f9" : "#1e293b" }}>{new Date(viewing.prescribedDate || viewing.createdAt).toLocaleDateString("en-US", { dateStyle: "long" })}</div>
                </div>
                <div className="dm-detail-panel" style={{ padding: 12, background: dark ? "#0f172a" : "#f8fafc", borderRadius: 12, border: `1px solid ${dark ? "#1e293b" : "#f1f5f9"}` }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", marginBottom: 4 }}>Status</div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: statusStyle[viewing.status]?.color || (dark ? "#f1f5f9" : "#1e293b") }}>{(viewing.status || "active").toUpperCase()}</div>
                </div>
              </div>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {viewing.medicines?.map((m, i) => (
                    <div className="dm-detail-panel" key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: dark ? "#0f172a" : "#f8fafc", border: `1px solid ${dark ? "#1e293b" : "#f1f5f9"}`, borderRadius: 16 }}>
                      <div className="dm-icon-chip" style={{ width: 36, height: 36, borderRadius: 10, background: dark ? "#111827" : "#fff", border: `1px solid ${dark ? "#334155" : "#e2e8f0"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <Pill size={18} color="#10b981" />
                      </div>
                      <div>
                        <div className="dm-soft-text" style={{ fontSize: 15, fontWeight: 600, color: dark ? "#f1f5f9" : "#0f172a" }}>{m.name}</div>
                        <div className="dm-soft-muted" style={{ fontSize: 13, color: dark ? "#94a3b8" : "#64748b", marginTop: 2 }}>{[m.dosage, m.frequency, m.duration].filter(Boolean).join(" • ")}</div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <div className="dm-warning-panel" style={{ background: dark ? "rgba(245,158,11,0.1)" : "#fffbeb", padding: 16, borderRadius: 16, border: `1px solid ${dark ? "rgba(245,158,11,0.2)" : "#fde68a"}`, display: "flex", gap: 12 }}>
                    <AlertCircle size={18} color={dark ? "#f59e0b" : "#d97706"} style={{ flexShrink: 0, marginTop: 2 }} />
                    <div style={{ fontSize: 14, color: dark ? "#fbbf24" : "#92400e", lineHeight: 1.6 }}>{viewing.advice}</div>
                  </div>
                </div>

              <div className="dm-detail-panel" style={{ padding: 16, background: dark ? "#0f172a" : "#f8fafc", borderRadius: 16, border: `1px solid ${dark ? "#1e293b" : "#f1f5f9"}`, display: "flex", alignItems: "center", gap: 12 }}>
                <User size={18} color="#64748b" />
                <div style={{ fontSize: 14, color: dark ? "#86efac" : "#589c65ff" }}>
                  Prescribed by <strong style={{ color: dark ? "#10b981" : "#33db5dff" }}>{viewing.doctorName || viewing.doctorId?.name || "Dr. Unknown"}</strong>
                </div>
              </div>
            </div>

            <div className="dm-modal-footer" style={{ padding: "16px 24px", borderTop: `1px solid ${dark ? "#1e293b" : "#f1f5f9"}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              {user?.role === "patient" && viewing.status === "active" ? (
                <button onClick={() => handleRefill(viewing._id)} disabled={refillLoading[viewing._id]} style={{ padding: "10px 20px", background: dark ? "rgba(16,185,129,0.15)" : "#f0fdf4", color: "#10b981", border: `1px solid ${dark ? "rgba(16,185,129,0.3)" : "#86efac"}`, borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
                  {refillLoading[viewing._id] ? "Requesting…" : <><Clock size={16} /> Request Refill</>}
                </button>
              ) : <div></div>}
              <button onClick={() => setViewing(null)} style={{ padding: "10px 24px", background: "#10b981", color: "#fff", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Close</button>
            </div>
          </div>
        </div>
      )}
